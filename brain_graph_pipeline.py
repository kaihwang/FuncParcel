...
def compare_full_v_partial_corr(output_path):
	...
	Full_graph = recursive_network_partition(matrix=Full_Ave_consensus, min_cost=.01,max_cost=0.25, min_community_size=5 ,min_weight=2)
	Partial_graph = recursive_network_partition(matrix=Partial_Ave_consensus, min_cost=.01,max_cost=0.25, min_community_size=5 ,min_weight=2)
...
def cluster_group_consensus_matrices(file_pattern):
	...
	graph = recursive_network_partition(matrix=AveMat, min_cost=.01,max_cost=0.25, min_community_size=5 ,min_weight=2)
	return graph
...
def write_group_consensus_cluste_results():
	...
	### cluster across averaged consensus matrices
