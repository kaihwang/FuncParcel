...
plot(kplot)
ggsave(filename = 'PC_classification_kernal.pdf', plot = kplot, units = c("in"),width=2.8, height=2,dpi=300) 
...
kplot <-kplot + coord_cartesian(ylim=c(0,.42), xlim=c( -4.2, 8.2))  
plot(kplot)
ggsave(filename = 'WMD_classification_kernal.pdf', plot = kplot, units = c("in"),width=2.8, height=2,dpi=300) 
...
kplot <-kplot+ geom_vline(data =parceldf, aes(xintercept=parcelmean), size=1)  + coord_cartesian(ylim=c(0,1)) + facet_wrap(~ Functional.Network)
plot(kplot)
ggsave(filename = 'kernal_wmd_by_network.pdf', plot = kplot, units = c("in"),width=6, height=5,dpi=300) 
...
kplot <-kplot + stat_density(aes( group = Functional.Network,fill = Functional.Network ), alpha=0.7, position="identity") 
kplot <-kplot + theme_grey(base_size = 8) +scale_fill_manual(values=CI_colors ) + theme(legend.position="none")
kplot <-kplot + xlim(0,1) + facet_wrap(~ Functional.Network)
parcelmean<-c(.60, .62, .94, .785, .786, .76, .91, .68, .84)
Functional.Network<-c("CO","DM", "FP", "latO", "mO", "mT", "sFP", "SM", "T")
parceldf<- data.frame(parcelmean,Functional.Network)
kplot <-kplot+ geom_vline(data =parceldf, aes(xintercept=parcelmean), size=1) + facet_wrap(~ Functional.Network) + coord_cartesian(ylim=c(0,5))
plot(kplot)
ggsave(filename = 'kernal_pc_by_network.pdf', plot = kplot, units = c("in"),width=6, height=5,dpi=300) 
